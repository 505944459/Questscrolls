        ...
        if(strings.length < 4 && strings.length > 0){ //首先 指令的自变量小于4 如果大于4 则一定是错误的所以不继续判定
            ...
            switch (strings[0]){    //开始对指令进行处理
                ...
                case "help": {  //帮助文档
                    ...
                    Zhu.sendMessage(commandSender,"§8[Questscrolls] 帮助文档");
                    ...
                    Zhu.sendMessage(commandSender,"§a/questscrolls list                       §7--  可用的任务卷轴");
                    Zhu.sendMessage(commandSender,"§a/questscrolls query                      §7--  进入调试模式");
                    Zhu.sendMessage(commandSender,"§a/questscrolls reload                     §7--  重载配置文件");
                    Zhu.sendMessage(commandSender,"§a/questscrolls general <Name> <player>    §7--  完成一个自定义任务");
                    Zhu.sendMessage(commandSender,"§cgeneral 不可作为独立的指令使用,而是作为一种任务行为使用!");
                    Zhu.sendMessage(commandSender,"§c具体使用方式和优点,请看默认提供的任务卷轴的介绍!");
                ...
                case "general":{ //通用任务
                    ...
                    //通用任务
                    new MainBusiness().Task("general",strings[1],Zhu.giveplayer(strings[2]),1);
