...
public class QuestscrollsPapi extends PlaceholderExpansion {
    ...
    public String onPlaceholderRequest(Player player, String identifier) { //核心部分
        ...
        if(identifier.equalsIgnoreCase("Task")){ //变量名 .xxx 中的 xxx
            ...
            //有吗?
            if(Zhu.ExecuteTask.containsKey(player.getName())){
                //缓存区有记录 就返回
                return Zhu.ExecuteTask.get(player.getName());
            }else{
                //无则返回默认值
